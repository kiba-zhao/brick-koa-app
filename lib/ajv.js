...
const { set, isString, isObject } = require('lodash');
const { provide, middleware, inject } = require('brick-engine');
...
function middlewareFactory(opts, item, routeOpts) {
  ...
  assert(v, '[brick-koa-app ajv] validate Error: validate schema not found');
